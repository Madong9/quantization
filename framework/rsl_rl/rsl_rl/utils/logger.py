...
class Logger:
    ...
    def log(
        ...
        if self.writer is not None:
            ...
            # Log model parameter distributions and summary statistics.
            self._log_model_parameters(actor, "Policy/Actor", it)
            self._log_model_parameters(critic, "Policy/Critic", it)
    ...
    def _log_model_parameters(self, model: torch.nn.Module | None, tag_prefix: str, step: int) -> None:
        """Log histograms and scalar summaries for model parameters."""
        ...
        for name, param in model.named_parameters():
            ...
            self.writer.add_histogram(f"{tag_prefix}/Histogram/{safe_name}", tensor, step)
            self.writer.add_scalar(f"{tag_prefix}/Mean/{safe_name}", tensor.mean().item(), step)
            self.writer.add_scalar(f"{tag_prefix}/Std/{safe_name}", tensor.std(unbiased=False).item(), step)
            self.writer.add_scalar(f"{tag_prefix}/Norm/{safe_name}", tensor.norm().item(), step)
            self.writer.add_scalar(f"{tag_prefix}/AbsMax/{safe_name}", tensor.abs().max().item(), step)
